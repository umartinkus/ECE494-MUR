...
uint8_t bar30_setup(i2c_master_bus_handle_t bus_handle,i2c_master_dev_handle_t bar30_handle){
    ...
    uint8_t error = 0;
    i2c_master_transmit(bar30_handle, &cmd, 1, I2C1_MASTER_TIMEOUT_MS);
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        i2c_master_transmit_receive(bar30_handle, &cmd, 1, data_buffer, 2, I2C1_MASTER_TIMEOUT_MS);
...
static void bar30_read_raw(i2c_master_dev_handle_t bar30_handle, uint8_t* dataBuffer){
    ...
    i2c_master_transmit(bar30_handle, &cmd, MS5837_I2C_CMD_SIZE, I2C1_MASTER_TIMEOUT_MS);
